  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setRawDishes((data || []).filter((dish: any) => dish.is_todays_special));
      } catch (err) {
      ...
      <div className="mx-auto w-full max-w-sm px-4 pt-6">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-[13px] text-[color:var(--brand-gold-muted)]">Loading specials...</p>
          </div>
        ) : dishes.length === 0 ? (
          <div className="flex flex-col items-center py-20 text-center">
            ...
            <p className="font-serif text-[18px] text-[color:var(--brand-gold)]">No specials today</p>
            <p className="mt-1 text-[13px] text-[color:var(--brand-gold-muted)]">Check back soon for our chef's picks!</p>
          </div>
        ) : (
