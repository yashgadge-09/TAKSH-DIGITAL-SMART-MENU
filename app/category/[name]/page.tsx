...
export default function CategoryPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  const { name: categoryNameRaw } = use(params);
    ...
    const loadData = async () => {
      try {
        ...
        const mappedDishes = (data || [])
          .filter((dish: any) => isSameCategory(dish.category, categoryName))
          .map((dish: any) => ({
            ...dish,
        ...
        <header className="px-4 pt-5 pb-2">
          <div className="mx-auto max-w-sm grid grid-cols-[40px_1fr_40px] items-center gap-3">
            ...
            <div className="text-center">
              <h1 className="font-serif text-[20px] leading-none tracking-wide text-[color:var(--brand-gold)] capitalize">
                {categoryName}
              </h1>
      ...
      <div className="mx-auto w-full max-w-sm px-4 pt-6">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-[13px] text-[color:var(--brand-gold-muted)]">Loading dishes...</p>
          </div>
        ) : dishes.length === 0 ? (
          <div className="flex flex-col items-center py-20 text-center">
            ...
            <p className="mt-1 text-[13px] text-[color:var(--brand-gold-muted)]">Check back soon for more!</p>
          </div>
        ) : (
